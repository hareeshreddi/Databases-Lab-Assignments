...
void push(struct Node** head_ref,const char* temp1,const char* temp2,const char* temp3,const char* temp4,char* temp,struct Node2* head2)
{
	if(search(head_ref,temp2,temp,head2)==0)
	{
		...
		struct Node* n = (struct Node*) malloc(sizeof(struct Node));
	  	...
	  	n->serial_number=(char*)malloc(sizeof(char)*strlen(temp1)+100);
	  	n->roll_number=(char*)malloc(sizeof(char)*strlen(temp2)+1000);
	  	n->name=(char*)malloc(sizeof(char)*strlen(temp3)+100);
	  	n->email=(char*)malloc(sizeof(char)*100);
	  	//n->subjects=(char*)malloc(sizeof(temp)+1000);
	  	strcpy(n->serial_number,temp1);
	  	strcpy(n->roll_number,temp2);
	  	strcpy(n->name,temp3);
	  	if(temp4!=NULL)
	  		strcpy(n->email,temp4);
	  	printf("reyrey::%s and %s\n",temp4,n->email );
...
void printList1(struct Node1 *head)
{
    ...
    while (head!= NULL)
    {
        ...
        fprintf(ff1,"insert into ett values ('%s','%s','%s','%s');\n",head->course_id,head->exam_date,head->start_time,head->end_time);
        fprintf(ff1,"insert into ett_temp values ('%s','%s','%s','%s');\n",head->course_id,head->exam_date,head->start_time,head->end_time);
        fprintf(ff1,"insert into ett_clone values ('%s','%s','%s','%s');\n",head->course_id,head->exam_date,head->start_time,head->end_time);
        head = head->next;
    }
...
void printList2(struct Node2 *head)
{
    ...
    while (head!= NULL)
    {
        ...
        fprintf(ff2,"insert into cc_temp values ('%s','%s');\n",head->course_id,head->no_of_credits);
        fprintf(ff2,"insert into cc_clone values ('%s','%s');\n",head->course_id,head->no_of_credits);
  		head = head->next;
    }
...
void printList(struct Node *head)//only printing if credits are greater than 40
{
    ...
    while (head!= NULL)
    {
    	...
    	while(x>=0)
    	{
        printf("INSERTING::%s %s %s %s %s\n",head->subject[x],head->serial_number,head->roll_number,head->name,head->email);
        fprintf(ff3,"insert into cwsl values (%d,%d,'%s','%s','%s');\n",atoi(head->serial_number),atoi(head->roll_number),head->name,head->email,head->subject[x]);
        fprintf(ff3,"insert into cwsl_temp values ('%d','%d','%s','%s','%s');\n",atoi(head->serial_number),atoi(head->roll_number),head->name,head->email,head->subject[x]);
        fprintf(ff3,"insert into cwsl_clone values ('%d','%d','%s','%s','%s');\n",atoi(head->serial_number),atoi(head->roll_number),head->name,head->email,head->subject[x]);
    	x--;
    	}
  		head = head->next;
    }
